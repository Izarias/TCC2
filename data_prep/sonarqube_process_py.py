...
SONAR_ISSUES_URL = os.getenv("SONAR_ISSUES_URL")
...
def parse_snippet_id(component_string):
    ...
    Exemplo: "SnippetsPY:SnippetsPY/D-A07-EN-PY-C4.py" -> "D-A07-EN-PY-C4"
    """
...
def process_sonarqube_results():
    print(f"Buscando as issues diretamente da API do SonarQube...")
    try:
        response = requests.get(SONAR_ISSUES_URL, auth=(SONAR_TOKEN, ''), timeout=15)
        if response.status_code != 200:
            print(f"Erro ao acessar API do Sonar: Status {response.status_code}")
            return
        data = response.json()
    except Exception as e:
        print(f"Não foi possível conectar ao SonarQube local. Erro: {e}")
        return
    ...
    if not df.empty:
        ...
        print("\nNenhuma issue foi encontrada ou processada da API do SonarQube.")

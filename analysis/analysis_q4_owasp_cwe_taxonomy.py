...
def owasp_cwe_taxonomy_analysis():
    ...
    # PREPARAÇÃO E TRATAMENTO OWASP / CWE
    ...
    print("\n=======================================================")
    print("1. CATEGORIAS OWASP MAIS RECORRENTES (COM ANO)")
    print("=======================================================")
    ...
    top_10_owasp = top_10_owasp.sort_values(by='Frequência', ascending=True)

    # Criando o gráfico de barras horizontais
    bars = plt.barh(
        top_10_owasp['owasp_list'],
        top_10_owasp['Frequência'],
        color=['#64B5CD', '#CCB974', '#8172B2', '#C44E52', '#55A868', '#4C72B0']
    )

    plt.bar_label(bars, padding=3)

    plt.title('Frequência por Categoria OWASP')
    plt.xlabel('Frequência')
    plt.ylabel('Categoria OWASP')
    ...
    print("\n=======================================================")
    print("2. CATEGORIAS CWE MAIS RECORRENTES")
    print("=======================================================")
    
    top_cwe = df_cwe_validas.groupby('cwe_list').size().reset_index(name='Frequência')
    top_cwe['Percentual (%)'] = (top_cwe['Frequência'] / top_cwe['Frequência'].sum()) * 100
    top_cwe = top_cwe.sort_values(by='Frequência', ascending=False).head(10)
    top_cwe['Percentual (%)'] = top_cwe['Percentual (%)'].round(2)
    
    print(top_cwe.to_string(index=False, header=['Categoria CWE', 'Frequência', 'Percentual (%)']))

    # Ordenando de forma crescente
    top_cwe = top_cwe.sort_values(by='Frequência', ascending=True)
    ...
    bars_cwe = plt.barh(
        ...
        top_cwe['Frequência'],
        color=[
            '#64B5CD', '#CCB974', '#8172B2', '#C44E52', '#55A868', 
            '#4C72B0', '#EBB7F5', '#9370DB', '#FFA07A', '#20B2AA'
        ]
    )

    plt.bar_label(bars_cwe, padding=3)

    plt.title('Frequência por Categoria CWE')
    plt.xlabel('Frequência')
    plt.ylabel('Categoria CWE')
    ...
    print(f"Categorias do OWASP Top 10 identificadas: {categorias_owasp_reais}")
    print(f"Total de categorias atingidas: {qtd_categorias} de 10")
